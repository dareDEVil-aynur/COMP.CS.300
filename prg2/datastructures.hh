...
// The part(s) where it was utilized: get_any_path
...
public:
    ...
    // Estimate of performance: O(n)
    // Short rationale for estimate: The function iterates through all connections of the given affiliation
    std::vector<Connection> get_connected_affiliations(AffiliationID id);

    // Estimate of performance: O(n)
    // Short rationale for estimate: The function iterates through all affiliations and their connections.
    std::vector<Connection> get_all_connections();

    // Estimate of performance: O(n^2)
    // Short rationale for estimate: BFS traverses through affiliations and their connections
    ...
    // Short rationale for estimate: Dijkstra's algorithm has a complexity of O(V + E * log(V)), where V is the number of vertices and E is the number of edges in the graph.
...
private:
    struct Affiliation
    {
        Name name;
        Coord coord;
        std::vector<AffiliationID> connected_affiliations;
    };
    ...
    std::unordered_map<Coord, AffiliationID, CoordHash> coordToAffiliation;
    std::unordered_map<AffiliationID, Affiliation> affiliations;
    std::unordered_map<PublicationID, Publication> publications;
    Distance calculate_distance(AffiliationID aff1, AffiliationID aff2);
};
